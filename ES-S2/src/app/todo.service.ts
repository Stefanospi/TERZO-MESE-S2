...
export class TodoService {
  ...
  todoWithAuthor: iPost[] = [];
  userWithTodo: iUser[] = [];

  constructor(private userSvc: UsersService) {}

  /* TODO CON AUTORE E DESCRIZIONE */
  postConAutore(): iPost[] {
    return (this.todoWithAuthor = this.todoList.map((p) => {
      let author = this.userSvc.users.find((u) => u.id == p.userId);
      if (author) {
        p.user = author;
      } else {
  ...
  /* FUNZIONE PER USERS */
  ritornaTodoUsers(): iUser[] {
    return (this.userWithTodo = this.userSvc.users.map((user) => {
      let todoUserId = this.todoList.filter((todo) => todo.userId == user.id);
      if(todoUserId){
        user.todo = todoUserId;
      }
      return user;
    }));
